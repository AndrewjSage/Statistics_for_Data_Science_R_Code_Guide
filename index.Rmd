...
### Bootstrap for Difference in Proportion Supporting Candidates

Bootstrap difference in proportion supporting each 2020 candidate, using results from Washington Post Poll in September 2020. 
...
Difference[i] <- (sum(BootstrapSample$Vote=="Biden") - 
                    sum(BootstrapSample$Vote=="Trump"))/702
}
...
for (i in 1:10000){
  # sample 33 northern lakes
NLakes <- sample_n(FloridaLakes %>% filter(Location=="N"), 33, replace=TRUE)   
 # sample 20 southern lakes
SLakes <- sample_n(FloridaLakes %>% filter(Location=="S"),  20, replace=TRUE)  
...
# record b1 (note indexing in coefficents starts at 1, so b0=coef(M)[1], and b1=coef(M)[2], etc.)
b1[i] <- coef(M)[2] 
}
...
#(note indexing in coefficents starts at 1, so b0=coef(M)[1], and b1=coef(M)[2], etc.)
...
```{r}
Boot_Dist_Plot<- ggplot(data=Vote_BootstrapResults, aes(x=Difference)) + 
...
```{r}
Estimate <- 0.50-0.46 #difference in proportions from original sample
```
